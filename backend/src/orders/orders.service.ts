...
export class OrdersService {
  ...
  private buildTelegramRestaurantOrderPayload(orderRow: {
    ...
    return {
      ...
      serviceFee: Number((orderRow as any).serviceFee ?? 0),
      deliveryFee: Number((orderRow as any).deliveryFee ?? 0),
      customerTotal:
        Math.round(
          (Number(orderRow.subtotal ?? 0) + Number((orderRow as any).deliveryFee ?? 0)) * 100,
        ) / 100,
  ...
  private buildTelegramCourierOrderPayload(orderRow: {
    ...
    return {
      ...
      restaurantName: orderRow.restaurant?.name ?? '—',
      total: this.courierTelegramFoodTotalUzs(orderRow.items),
      subtotal: this.courierTelegramFoodTotalUzs(orderRow.items),
      customerName: orderRow.customer?.name ?? '',
      phone,
      lat: orderRow.address?.latitude,
      lng: orderRow.address?.longitude,
  ...
  /**
   * Admin panel va bot holatlari bir xil ko'rinishi uchun:
   * status o'zgarganda restoran Telegram chat(lar)iga qisqa sinxron xabar yuboramiz.
   */
  private notifyRestaurantTelegramStatusChanged(orderId: string, status: OrderStatus): void {
    const disableTelegramEnv = process.env.DISABLE_TELEGRAM_NOTIFY === 'true';
    if (disableTelegramEnv) return;
    const notifyUrl = (process.env.TELEGRAM_BOT_NOTIFY_URL ?? '').trim();
    if (!notifyUrl) return;
    ...
    void (async () => {
      const row = await this.prisma.order.findUnique({
        ...
        select: {
          shortCode: true,
          status: true,
          restaurant: { select: { name: true, telegramChatId: true } },
          subtotal: true,
          deliveryFee: true,
        },
      });
      if (!row?.restaurant?.telegramChatId) return;
      const chatIds = String(row.restaurant.telegramChatId)
        .split(/[,;\n\r\s]+/g)
        .map((s) => s.trim())
        .filter(Boolean);
      if (chatIds.length === 0) return;
      ...
      await Promise.all(
        chatIds.map(async (chatId) => {
          try {
            ...
            this.logger.warn(
              `[telegram] status sync failed chatId=${String(chatId).slice(0, 30)} order=${orderId} err=${
                e instanceof Error ? e.message : String(e)
              }`,
            );
          }
